    ...
    load( gl ) {
        ...
        if (material.hasTexture()) {
            ...
            for (let i = 0, j = 0; i < vertex_positions.length; i+=3, j+=2) {
                vertex_data.push(vertex_positions[i]); // Px
                vertex_data.push(vertex_positions[i+1]); // Py
                vertex_data.push(vertex_positions[i+2]); // Pz
    
                vertex_data.push(vertex_normals[i]); // Nx
                vertex_data.push(vertex_normals[i+1]); // Ny
                vertex_data.push(vertex_normals[i+2]); // Nz

                vertex_data.push(vertex_tangents[i]); // Tx
                vertex_data.push(vertex_tangents[i+1]); // Ty
                vertex_data.push(vertex_tangents[i+2]); // Tz

                vertex_data.push(vertex_texture_coords[j]); // U
                vertex_data.push(vertex_texture_coords[j+1]); // V
                // vertex_data.push(vertex_texture_coords[i+2]); // TCz
            }

        } else {
            // merge when there are no textures
